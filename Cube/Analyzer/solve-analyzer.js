(() => {
  ...
  // Milestones are first achieved in real time. A later algorithm can temporarily
  // disturb a completed Cross/F2L/OLL; requiring it to stay solved until the timer
  // stops shifts every boundary into PLL (the 2026-09-23 GAN trace reproduced this).
  function firstReached(states, predicate, from = 0, through = states.length - 1) {
    for (let i = Math.max(0, from); i <= Math.min(through, states.length - 1); i++) {
  ...
  function detectCFOP(states) {
    ...
    // Identify the first full F2L completion *before* the last move, then the
    // earliest earlier Cross that leads to four independently evidenced slot
    // completions. Do not demand that Cross/F2L remain intact throughout LL.
    // The final solved state alone is insufficient to prove the method.
    let chosen = null;
    for (let f = 0; f < n && !chosen; f++) {
      if (!f2l(states[f])) continue;
      for (let cr = 0; cr <= f; cr++) {
        if (!cross(states[cr])) continue;
        const slots = confirmF2LSlots(states, cr, f);
        if (!slots) continue;
        chosen = { cross: cr, f2lEnd: f, slots };
        break;
      }
    }
    if (!chosen) return null;

    // OLL = first state with both F2L and LL orientation complete *after* F2L;
    // an algorithm may break and restore those conditions during PLL.
    const ollEnd = firstReached(states, oll, chosen.f2lEnd, n);
    if (ollEnd < 0) return null;
    return {
      marks: [chosen.cross, chosen.f2lEnd, ollEnd, n],
  ...
  function detectRoux(states) {
    const n = states.length - 1;
    if (n < 2 || !isSolved(states[n])) return null;
    const sb = firstReached(states, secondBlock, 0, n - 1);
    if (sb < 0) return null;
    const fb = firstReached(states, firstBlock, 0, sb);
    if (fb < 0) return null;
    const cm = firstReached(states, cmll, sb, n);
    if (cm < 0) return null;
  ...
  function detectZZ(states) {
    ...
    for (let f = 0; f < n && !chosen; f++) {
      if (!f2l(states[f])) continue;
      const eo = firstReached(states, eoCross, 0, f);
      if (eo >= 0) chosen = { eo, f };
    }
    if (!chosen) return null;
    return { marks: [chosen.eo, chosen.f, n], confidence: 'detected' };
  }
  ...
  // AUF recognition (2026-09-24): adjustment-layer (the user's "U" layer =
  // opposite the detected Cross bottom face, not the literal U of the recording
  // frame) turns are classified as pre-alg AUF (before execution starts, part
  // of the recognition window) or post-alg AUF (after the last non-adjustment
  // turn, the finishing AUF). Adjustment turns in between belong to the
  // algorithm itself (e.g. U moves inside a PLL) and are not AUF.
  function recognizeAUF(segment, execStart, adjRe) {
    let lastCore = -1;
    for (let i = segment.length - 1; i >= 0; i--) {
      const m = String(segment[i] || '').trim();
      if (!m || ROTATION_RE.test(m) || adjRe.test(m)) continue;
      lastCore = i; break;
    }
    const pre = [], post = [];
    for (let i = 0; i < segment.length; i++) {
      const m = String(segment[i] || '').trim();
      if (!m || ROTATION_RE.test(m) || !adjRe.test(m)) continue;
      if (i < execStart) pre.push(m);
      else if (i > lastCore) post.push(m);
    }
    return { pre, post, count: pre.length + post.length };
  }
  ...
  function stageTiming(startIndex, endIndex, moves, timestamps, options = {}) {
    ...
    let recognition=0,turns=countTurns(moves,startIndex,endIndex),aufInfo=null;
    if(!options.noRecognition && endIndex>startIndex){
      ...
      // F2L/OLL/PLL rule (2026-09-24): a turn of the adjustment layer before
      // execution begins is reaction/AUF-style adjustment, not the start of the
      // formula — only a non-adjustment turn can trigger execution, and such
      // turns inside the recognition window are not counted as formula turns.
      // Moves after execution started remain part of the algorithm.
      // The adjustment layer is the face opposite the detected Cross (the user's
      // "U layer"), not necessarily the literal U face of the recording frame.
      const ADJ_RE=new RegExp('^'+(options.adjFace||'U')+"(?:2|')?$");
      ...
      for(let i=0;i<segment.length;i++){
        ...
        if(options.uAsAdjustment&&ADJ_RE.test(m))continue;
      ...
      const triggered=executionIndex>=0;
      if(executionIndex<0)executionIndex=segment.length-1;
      recognition=Math.max(0,Math.min(time,times[executionIndex]-start));
      if(options.uAsAdjustment){
        const execStart=triggered?executionIndex:segment.length;
        const auf=recognizeAUF(segment,execStart,ADJ_RE);
        if(auf.count)aufInfo=auf;
        turns=0;
        for(let i=0;i<segment.length;i++){
          const m=String(segment[i]||'').trim();
          if(!m||ROTATION_RE.test(m))continue;
          if(i<execStart&&ADJ_RE.test(m))continue;
          turns++;
        }
      }
    }
    const out={time,turns,recognition,execution:Math.max(0,time-recognition),startIndex,endIndex,moves:moves.slice(startIndex,endIndex)};
    if(aufInfo)out.auf=aufInfo;
    return out;
  }
  ...
  const ao = v => (3-(v%3))%3;
  // Report the original face used for Cross, not merely a library-specific
  // rotation index. cstimer's solved cubie positions are independent of color.
  const CROSS_FACE_EDGES = {
    U:[0,1,2,3], R:[0,4,8,11], F:[1,5,8,9],
    D:[4,5,6,7], L:[2,6,9,10], B:[3,7,10,11]
  };
  const FACE_F2L_PAIRS = {
    U:[[0,8],[1,9],[2,10],[3,11]],
    R:[[3,3],[0,1],[4,5],[7,7]],
    F:[[0,0],[1,2],[5,6],[4,4]],
    D:[[4,8],[5,9],[6,10],[7,11]],
  ...
  function originalCrossFace(states, crossIndex, f2lIndex) {
    const candidates = Object.keys(CROSS_FACE_EDGES).filter(face =>
      CROSS_FACE_EDGES[face].every(i => edgeSolved(states[crossIndex], i)) &&
      CROSS_FACE_EDGES[face].every(i => edgeSolved(states[f2lIndex], i)) &&
      FACE_F2L_PAIRS[face].every(([corner, edge]) =>
        cornerSolved(states[f2lIndex],corner) && edgeSolved(states[f2lIndex],edge))
    );
  ...
  function makeCFOPSteps(found, moves, timestamps, crossFace) {
    const [cr, f2lEnd, ollEnd, n] = found.marks;
    // Stage-start adjustment layer = the face opposite the detected Cross (the
    // user's "U layer", derived from the judged bottom face). Falls back to the
    // literal U face when the cross face cannot be determined.
    const adjFace = OPPOSITE_FACE[crossFace] || 'U';
    const adjOpts = { uAsAdjustment: true, adjFace };
    const steps = [];
    steps.push({ name: 'Cross', label: 'Cross', ...stageTiming(0, cr, moves, timestamps, { noRecognition: true }) });

    const f2lAggregate = { name: 'F2L', label: 'F2L', ...stageTiming(cr, f2lEnd, moves, timestamps, adjOpts) };
    ...
    slotEvents.forEach((event, order) => {
      const st = { name: 'F2L', label: `F2L ${order + 1}`, slotIndex: order + 1, physicalSlot: event.slot, ...stageTiming(prev, event.index, moves, timestamps, adjOpts) };
    ...
    const ollStep = { name: 'OLL', label: 'OLL', ...stageTiming(f2lEnd, ollEnd, moves, timestamps, adjOpts) };
    attachCase(ollStep, ollCase(found.states[f2lEnd]));
    steps.push(ollStep);

    const pllStep = { name: 'PLL', label: 'PLL', ...stageTiming(ollEnd, n, moves, timestamps, adjOpts) };
  ...
  function analyze({ method = 'CFOP', startFacelet, moves = [], timestamps = [], totalTime = 0, snapshots = [], rawSolutionSequence = [] } = {}) {
    ...
    // Cross face (in the original recording frame) is needed both by the stage
    // split (opposite face = stage-start adjustment layer) and the report frame.
    const crossFace = normalizedMethod === 'CFOP' || normalizedMethod === 'ZZ'
      ? originalCrossFace(states, found.marks[0], found.marks[1]) : null;
    const steps = normalizedMethod === 'CFOP'
      ? makeCFOPSteps(found, cleanMoves, cleanTs, crossFace)
